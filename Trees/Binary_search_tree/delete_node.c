// LDelete a element in the binary search tree
...
struct Node *delete(struct Node *temp,int value)
{
    ...
    else if(value == temp->data)
    {
        ...
        {
            temp->data = pred(temp->left,temp->data);  // find the predecessor value
            temp->left = delete(temp->left,value);
            return temp;
        }
    }
...
int pred(struct Node *temp,int value)
{
    ...
    // swap the values
    int num = temp->data; // store the predecessor
    temp->data = value;  
    return num;
}
